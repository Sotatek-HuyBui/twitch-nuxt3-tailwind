    ...
    <div
        ...
        <div class="flex-1 flex items-center">
            ...
            <div v-if="token" class="h-full flex items-center mr-8" :class="{
                ...
                <p class="cursor-pointer font-medium hover:text-customPrimary-1 dark:hover:text-customPrimary-0 text-[18px]"
                    :class="{
            ...
            <UDropdown :items="generalItems" :popper="{ placement: 'bottom-start' }">
                <UTooltip text="More options"
                    ...
                    <img v-if="colorMode.value !== 'dark'" src="~/assets/three-dot-menu.png"
                        class="w-2 h-2 sm:w-4 sm:h-4 cursor-pointer" alt="Three dot menu" />
                    <img v-else src="~/assets/three-dots-menu-white.png" class="w-2 h-2 sm:w-4 sm:h-4 cursor-pointer"
                        alt="Three dot menu" />
                </UTooltip>
            </UDropdown>
        </div>
        ...
        <div class="flex-1 flex items-center justify-end">
            <div class="relative sm:hidden">
                <img src="~/assets/search.png" class="w-3 h-3 sm:hidden cursor-pointer mr-2 sm:mr-3" alt="Logo"
            ...
            <UTooltip text="Prime gaming benefits"
                ...
                <svg v-if="colorMode.value !== 'dark'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="14"
                    height="14" version="1.1" xmlns:xlink="http://www.w3.org/1999/xlink"
                    ...
                    <g transform="matrix(1,0,0,1,0,0)">
                        <g id="_01_align_center" data-name="01 align center">
                            <path
                                ...
                                fill="#525252" fill-opacity="1" data-original-color="#000000ff" stroke="none"
                                stroke-opacity="1" />
                        </g>
                    </g>
                </svg>
                <!-- <img
                    ...
                    alt="Logo"
                /> -->
                <svg v-else xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="14" height="14" version="1.1"
                    xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:svgjs="http://svgjs.dev/svgjs">
                    <g transform="matrix(1,0,0,1,0,0)">
                        <g id="_01_align_center" data-name="01 align center">
                            <path
                                d="M21,21.707H3a3,3,0,0,1-3-3V2.293l6,6,6-6,6,6,6-6V18.707A3,3,0,0,1,21,21.707ZM2,7.121V18.707a1,1,0,0,0,1,1H21a1,1,0,0,0,1-1V7.121l-4,4-6-6-6,6Z"
                                fill="#bfbfbf" fill-opacity="1" data-original-color="#000000ff" stroke="none"
                                stroke-opacity="1" />
                        </g>
                    </g>
                </svg>
            ...
            <UTooltip text="Notifications" v-if="token"
                class="dark:hover:bg-darkButtonHover hover:bg-lightButtonHover mr-2 sm:mr-3 p-2 rounded-md">
                <img v-if="colorMode.value !== 'dark'" src="~/assets/bell.png"
                    class="w-3 h-3 sm:w-4 sm:h-4 cursor-pointer" alt="Logo" />
                <img v-else src="~/assets/bell-white.png" class="w-3 h-3 sm:w-4 sm:h-4 cursor-pointer" alt="Logo" />
            </UTooltip>
            <UTooltip text="Whispers" v-if="token"
                class="dark:hover:bg-darkButtonHover hover:bg-lightButtonHover mr-2 sm:mr-3 p-2 rounded-md">
                <img v-if="colorMode.value !== 'dark'" src="~/assets/conversation.png"
                    class="w-3 h-3 sm:w-4 sm:h-4 cursor-pointer" alt="Logo" />
                <img v-else src="~/assets/conversation-white.png" class="w-3 h-3 sm:w-4 sm:h-4 cursor-pointer"
                    alt="Logo" />
            </UTooltip>
            <UTooltip text="Get bids" v-if="token"
                class="dark:hover:bg-darkButtonHover hover:bg-lightButtonHover mr-2 sm:mr-3 p-2 rounded-md">
                <img v-if="colorMode.value !== 'dark'" src="~/assets/shield.png"
                    class="w-4 h-4 sm:w-4 sm:h-4 cursor-pointer" alt="Logo" />
                <img v-else src="~/assets/shield-white.png" class="w-4 h-4 sm:w-4 sm:h-4 cursor-pointer" alt="Logo" />
            </UTooltip>
            <button v-if="token" type="button"
                ...
                " src="~/assets/low-battery.png" class="w-3 h-3 sm:w-4 sm:h-4 cursor-pointer mr-2 sm:mr-3"
                    alt="Logo" />
                <img v-if="token && colorMode.value !== 'light'" src="~/assets/low-battery-white.png"
                    class="w-3 h-3 sm:w-4 sm:h-4 cursor-pointer mr-2 sm:mr-3" alt="Logo" />
            ...
            <button v-if="!token" type="button" @click="onShowRegisterModal"
                ...
                {{ $t('Sign up') }} </button>
            <UDropdown :items="configItems" :popper="{ placement: 'bottom-start' }">
                <div v-if="!token">
                    <ColorScheme placeholder="" tag="span">
                        <template #placeholder>
                            <div class="h-2.5 bg-gray-200 rounded-full dark:bg-gray-700 w-6"></div>
                        </template>
                        <img :src="isLightTheme ? '/images/profile-user.png' : '/images/profile-user-white.png'"
                            class="w-3 h-3 sm:w-6 sm:h-6 cursor-pointer" alt="Logo" />
                    </ColorScheme>
                </div>
                <img v-else
